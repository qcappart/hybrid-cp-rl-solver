...
public:
    ...
    /* Constructor */
    TSPTW_DP(const OptionsTSPTW& opt): IntMinimizeScript(opt), tour_cost(*this, 0,
    100000) {
        ...
        auto to_run_module = py::module::import("to_run_TSPTW");;
        to_run = to_run_module.attr("ToRunTSPTW")(model_folder, m, opt.grid_size, opt.max_tw_gap, opt.max_tw_size, seed, mode);


        auto travel_time_python = to_run.attr("get_travel_time_matrix")();
        this->travel_time = travel_time_python.cast<std::vector<std::vector<double>>>();

        auto time_windows_python = to_run.attr("get_time_windows_matrix")();
        ...
        //branch(*this, travel_to, INT_VAR_NONE(), INT_VAL(&weighted_random_value_selector));
    ...
    static int value_selector(const Space& home, IntVar x, int i) {
        ...
        if (tsptw_dp.use_cache && predict_cache.find(key) != predict_cache.end()){
            ...
            py::list y_predict = tsptw_dp.to_run.attr("predict_dqn")(non_fixed_variables, last_visited);
...
//        py::list y_predict = tsp_dp.to_run.attr("predict")(fixed_variables, last_visited);
//        py::list y_predict = tsp_dp.to_run.attr("predict")(fixed_variables_python, last_visited);
//        q_values = y_predict.cast<std::vector<double>>();

//        //TODO: [Quentin] Actually, we should consider only the city that are not been selected yet AND not being pruned during any propagation
    ...
    static int probability_selector(const Space& home, IntVar x, int i) {
        ...
        if (tsptw_dp.use_cache && predict_cache.find(key) != predict_cache.end()){
            ...
            py::list y_predict = tsptw_dp.to_run.attr("predict_ppo")(non_fixed_variables, last_visited, tsptw_dp.temperature);
        ...
        //int selected_value = weighted_selector(tsp_dp.m, prob_values, x);
     ...
     static int nearest_neighbour_value_selector(const Space& home, IntVar x, int i) {
        //Nearest non-visited heuristic:
        const TSPTW_DP& tsptw_dp = static_cast<const TSPTW_DP&>(home);
        int last_visited = tsptw_dp.last_city[i].val();
        int best_q_value = get_nearest_non_visited(tsptw_dp.m, last_visited, tsptw_dp.travel_time, x);
        //cout << best_q_value << endl;
        return best_q_value;
    }

    static int get_nearest_non_visited(int n, int last_visited, std::vector<std::vector<double>> travel_time,
                                       IntVar cur_var){
    ...
    virtual void print(std::ostream& os) const {
        ...
        int elapsed_seconds = std::chrono::duration_cast<std::chrono::milliseconds> (end-start).count();
          if(tour_cost.val() < best_cost) {
            best_cost = tour_cost.val();
            os << this->m << "," << this->seed << "," << elapsed_seconds << "," << tour_cost << endl;
          }
...
/*
        cout << "---------------" << endl;
        cout << "execution time: " << elapsed_seconds << " ms" << endl;
        cout << "visited_cities: " << must_visit_city << endl;
        cout << "last_city: " << last_city << endl;
        cout << "travel_to: " << travel_to << endl;
        cout << "acc_cost: " << acc_cost << endl;
        cout << "tour_cost: " << tour_cost << endl;
        cout << "time: " << time << endl;
*/
    ...
    static int weighted_selector(int n, std::vector<double> q_values, IntVar cur_var) {
        std::array<double,20> normalized_q_values;
        double sum_q_value = 0;

        for (int i = 0; i < n; ++i) {

            if (cur_var.in(i)) {
                normalized_q_values[i] = q_values[i] + *std::min_element(q_values.begin(), q_values.end());

            }
            else {
                normalized_q_values[i] = 0;
            }

            sum_q_value +=  normalized_q_values[i];
        }

        for (int i = 0; i < n; ++i) {
            normalized_q_values[i] = normalized_q_values[i] / sum_q_value;

        }

        std::discrete_distribution<int> distribution(normalized_q_values.begin(), normalized_q_values.end());
        int action = distribution(generator);
        return action;
    }
...
int main(int argc, char* argv[]) {
    ...
    // opt.size(1); // must be set, does not use a flag in the command line
    opt.solutions(0); // Find best solution by default
    opt.seed(result["seed"].as<int>());
    opt.time(result["time"].as<int>());
    opt.d_l(result["d_l"].as<int>()); // LDS cutoff
//    opt.add(new Gecode::Driver::BaseOption("luby", "luby scaling factor"));
...
//    opt.parse(argc,argv);
    ...
    if(opt.model() == TSPTW_DP::RL_DQN || opt.model() == TSPTW_DP::NN_HEURISTIC) {
        //IntMinimizeScript::run<TSPTW_DP,LDS,OptionsTSPTW>(opt);
        Search::Options o;
        ...
        delete p;
        cout << "nb_cities,seed,time,tour_cost,num_nodes,num_fails,depth" << std::endl;
        while(TSPTW_DP* p = engine.next()) {
            ...
            if(cur_cost < best_cost) {
                ...
                cout << p->to_string() << "," << num_nodes << "," << num_fail << "," << depth << endl ;
            }
            delete p;
        }
        std::chrono::time_point<std::chrono::system_clock> end = std::chrono::system_clock::now();
