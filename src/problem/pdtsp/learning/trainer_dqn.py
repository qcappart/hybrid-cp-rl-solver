...
from src.problem.pdtsp.environment.environment import Environment
from src.problem.pdtsp.learning.brain_dqn import BrainDQN
...
class TrainerDQN:
    """
    Definition of the Trainer DQN for the PDTSP
    """
    ...
    def __init__(self, args):
        ...
        self.num_edge_feats = 5  # TODO: un-hardcode that

        self.reward_scaling = 0.001

        self.validation_set = PDTSP.generate_dataset(size=VALIDATION_SET_SIZE, n_city=self.args.n_city, m_commodity=self.args.m_commodity,
                                                     grid_size=self.args.grid_size, max_commodity_weight=self.args.max_commodity_weight,
                                                     capacity_looseness=self.args.capacity_looseness, seed=np.random.randint(10000),
    ...
    def run_episode(self, episode_idx, memory_initialization):
        ...
        instance = PDTSP.generate_random_instance(n_city=self.args.n_city, m_commodity=self.args.m_commodity, grid_size=self.args.grid_size,
                                                  max_commodity_weight=self.args.max_commodity_weight, capacity_looseness=self.args.capacity_looseness,
                                                  seed=-1, is_integer_instance=False)

        env = Environment(instance, self.num_node_feats, self.num_edge_feats, self.reward_scaling,
                          self.args.grid_size, self.args.max_commodity_weight)
