...
from src.problem.pdtsp.environment.pdtsp import PDTSP
from src.problem.pdtsp.learning.actor_critic import ActorCritic
...
class SolverBinding(object):
    ...
    def __init__(self, load_folder, n_city, m_commodity, grid_size, max_commodity_weight, capacity_looseness, seed, algorithm):
        """
        ...
        :param max_tw_gap: maximum single commodity weight
        ...
        self.capacity_looseness = capacity_looseness
        self.algorithm = algorithm
        self.seed = seed

        self.max_dist = np.sqrt(self.grid_size ** 2 + self.grid_size ** 2)

        self.instance =  PDTSP.generate_random_instance(n_city=self.n_city, m_commodity=self.m_commodity, grid_size=self.grid_size,
                                                        max_commodity_weight=self.max_commodity_weight, capacity_looseness=self.capacity_looseness,
    ...
    def get_distance_matrix(self):
        """
        Return the distance matrix (used in the c++ code)
        :return: The distance matrix
        """
        return self.instance.distances

    def get_pickup_constraints_matrix(self):
        """
        Return the pickup constraints matrix (used in the c++ code)
        :return: the pickup constraints matrix
        """

        return self.instance.pickup_constraints

    def get_capacity(self):
        """
        Return the capacity of the generated instance (used in the c++ code)
        :return: the capacity
        """
        returns self.instance.capacity
    ...
    def initialize_graph(self):
        ...
        tour_node_feats = [[0, 0] for i in range(g.number_of_nodes())]
    ...
    def update_graph_state(self, non_fixed_variables, last_visited):
        ...
        pos_node_feats = [[self.instance.x_coord[i] / self.grid_size,  # x-coord
                      self.instance.y_coord[i] / self.grid_size]
                      for i in range(self.input_graph.number_of_nodes())]  # y-coord

        commodity_node_feats = [self.instance.pickup_constraints[i] / self.max_commodity_weight for i in range(self.input_graph.number_of_nodes())]

        tour_node_feats = [[0 if i in non_fixed_variables else 1,  # 0 if it is possible to visit the node
                      1 if i == last_visited else 0]   # 1 if it is the last node visited
